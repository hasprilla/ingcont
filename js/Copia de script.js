        ...
        .then(function (response) {
            for (let index = 0; index < response.length; index++) {
              if(response[index].hijos > 0)
              {
                ...
                ariaexpanded = '';
                ariacontrols = '';
                eventoclick = `onclick="objetos('${response[index].codigo}'); recargar('${page_root + makeSlug(response[index].nombre) + "/"}')"`;
              }
...
function cargarSubMenu(id, nombre)
{
    const data = new URLSearchParams({
     ...
     fetch(urlBase + 'menu/', {
        ...
        .then(function (response) {
          for (let index = 0; index < response.length; index++) {
               $("#submenu"+nombre).append(`<a class="collapse-item" onclick="objetos('${response[index].id}'); recargar('${page_root + makeSlug(response[index].descripcion) + "/"}')">${response[index].descripcion}</a>`);
          }
        })
        .catch(function (err) {
        });
